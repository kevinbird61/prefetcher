...
void avx_prefetch_transpose(int *src,int *dst,int w,int h)
{
    for(int x=0; x<w; x+=8) {
        for(int y=0; y<h; y+=8) {
#define A_PFDIST  8
            _mm_prefetch(src+(y + A_PFDIST + 0) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 1) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 2) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 3) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 4) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 5) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 6) *w + x, _MM_HINT_T1);
            _mm_prefetch(src+(y + A_PFDIST + 7) *w + x, _MM_HINT_T1);
